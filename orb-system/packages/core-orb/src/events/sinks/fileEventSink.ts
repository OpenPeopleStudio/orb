 ...
 */
export class FileEventSink implements OrbEventSink {
  private eventsPath: string;
  private maxEventsPerFile: number = 1000;

  constructor(maxEventsPerFile: number = 1000) {
   ...
   */
  private getRelativePath(date: Date): string {
   ...
   */
  async emit(event: OrbEvent): Promise<void> {
    ...
    const existingEvents: OrbEvent[] = await readJson(relativePath, []);
  ...
  async query(filter: EventFilter): Promise<OrbEvent[]> {
    ...
    while (currentDate <= dateTo) {
      ...
      const events: OrbEvent[] = await readJson(relativePath, []);
   ...
   */
  private matchesFilter(event: OrbEvent, filter: EventFilter): boolean {
    if (filter.type) {
  ...
  async getStats(filter?: EventFilter): Promise<EventStats> {
    ...
    const byType: Partial<Record<OrbEventType, number>> = {};
    ...
    return {
      ...
      byType: byType as Record<OrbEventType, number>,
      byMode,
      byRole: byRole as Record<string, any>,
      mostUsedModes,
