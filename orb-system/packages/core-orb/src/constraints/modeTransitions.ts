...
export function validateModeTransition(
  ...
  if (context.device && !toModeDescriptor.defaultDevices.includes(context.device)) {
    ...
    recommendations.push(
      `${toMode} is optimized for: ${toModeDescriptor.defaultDevices.join(', ')}`
    );
  }
  ...
  if (!toModeDescriptor.defaultPersonas.includes(context.persona)) {
    ...
    recommendations.push(
      `${toMode} is optimized for: ${toModeDescriptor.defaultPersonas.join(', ')}`
    );
  }
  ...
  for (const constraintSet of constraintSets) {
    for (const constraint of constraintSet.constraints) {
      ...
      if (constraint.type !== 'mode_transition') continue;
      ...
      if (constraint.allowedModes && !constraint.allowedModes.includes(toMode)) {
        ...
        reasons.push(
          constraint.reason || `Transition to ${toMode} is restricted by constraint ${constraint.id}`
        );
        
        recommendations.push(
          `Allowed transitions from ${fromMode}: ${constraint.allowedModes.join(', ')}`
        );
      }
    }
  }
  ...
  const hasHardBlocks = triggeredConstraints.some(
    tc => tc.severity === 'critical' || tc.severity === 'error'
  );
 ...
 */
export function getModeTransitionDenialReasons(): Record<string, string> {
  return {
 ...
 */
export function getRecommendedMode(context: ActionContext): OrbMode {
