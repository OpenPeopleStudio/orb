...
function selectTone(
  ...
  userEmotion?: { emotion: string; intensity: number; valence: number }
): 'celebratory' | 'compassionate' | 'gentle' | 'encouraging' | 'analytical' {
...
function calculateAlignmentScore(context: ReflectionContext): number | null {
...
function generateTags(context: ReflectionContext): string[] {
  ...
  if (totalSpend < 200) tags.push('low_spend');
  
  // Time pattern tags
  if (context.timePatterns.lateNight > totalSpend * 0.2) tags.push('late_night_spending');
  if (context.timePatterns.weekend > totalSpend * 0.5) tags.push('weekend_heavy');
  
  // Category tags
  const topCategory = Object.entries(context.categoryBreakdown)
    .sort(([, a], [, b]) => b - a)[0]?.[0];
  if (topCategory) tags.push(`category_${topCategory}`);
  
  // Pattern tags
  const uniqueMerchants = Object.keys(context.merchantPatterns).length;
  if (uniqueMerchants < context.transactions.length * 0.3) tags.push('repetitive_merchants');
  if (uniqueMerchants > context.transactions.length * 0.8) tags.push('diverse_spending');
  
  // Alignment tags
  const alignmentScore = calculateAlignmentScore(context);
  if (alignmentScore && alignmentScore > 0.5) tags.push('aligned');
