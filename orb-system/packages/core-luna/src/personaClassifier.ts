 ...
 */
class InMemoryPersonaOverrideStore implements IPersonaOverrideStore {
  private overrides: Map<string, OrbPersona> = new Map();
  
  async getOverride(userId: string, sessionId?: string): Promise<OrbPersona | null> {
    const key = sessionId ? `${userId}:${sessionId}` : userId;
    return this.overrides.get(key) ?? null;
  }
  
  async setOverride(userId: string, persona: OrbPersona, sessionId?: string): Promise<void> {
    const key = sessionId ? `${userId}:${sessionId}` : userId;
    this.overrides.set(key, persona);
  }
  
  async clearOverride(userId: string, sessionId?: string): Promise<void> {
    const key = sessionId ? `${userId}:${sessionId}` : userId;
 ...
 */
export class RuleBasedPersonaClassifier implements IPersonaClassifier {
  private overrideStore: IPersonaOverrideStore;
   ...
   */
  async classifyPersona(context: PersonaContext): Promise<PersonaClassificationResult> {
    ...
    if (context.userId) {
      const override = context.userOverride ?? await this.overrideStore.getOverride(
        context.userId,
        context.sessionId
      );
      if (override) {
        ...
        return {
          ...
          distribution: {
            [override]: 1.0,
            ...Object.fromEntries(
              Object.values(OrbPersona)
                .filter(p => p !== override)
                .map(p => [p, 0])
            ),
          } as Record<OrbPersona, number>,
    ...
    const entries = Object.entries(distribution) as [OrbPersona, number][];
   ...
   */
  async setPersonaOverride(
    userId: string,
    persona: string,
    sessionId?: string
  ): Promise<void> {
    // Validate persona
    if (!Object.values(OrbPersona).includes(persona as OrbPersona)) {
      throw new Error(`Invalid persona: ${persona}`);
    }
    
    await this.overrideStore.setOverride(userId, persona as OrbPersona, sessionId);
  }
  ...
  private applyDeviceRule(
    device: OrbDevice,
    distribution: Record<OrbPersona, number>,
    signals: string[]
  ): number {
    let weight = 0;
  ...
  private applyTimeOfDayRule(
    timeOfDay: NonNullable<PersonaContext['timeOfDay']>,
    distribution: Record<OrbPersona, number>,
    signals: string[]
  ): number {
    let weight = 0;
 ...
 */
let defaultClassifier: RuleBasedPersonaClassifier | null = null;

/**
 * Get or create the default persona classifier
 */
export function getPersonaClassifier(): RuleBasedPersonaClassifier {
 ...
 */
export async function classifyPersona(context: PersonaContext): Promise<PersonaClassificationResult> {
