...
import {
  ...
  type IConstraintEvaluator,
  type RiskLevel,
  type ConstraintType,
  DEFAULT_CONSTRAINT_IDS,
 ...
 */
export class DefaultConstraintEvaluator implements IConstraintEvaluator {
  constructor(private store: IConstraintStore) {}
  ...
  async evaluateAction(
    ...
    const sets = constraintSets ?? await this.store.getConstraintSets(
      context.userId,
      context.currentMode,
    ...
    const triggeredConstraints: ConstraintEvaluationResult['triggeredConstraints'] = [];
    const reasons: string[] = [];
    const riskFactors: string[] = [];
    let decision: ConstraintEvaluationResult['decision'] = 'allow';
    let effectiveRisk: RiskLevel = context.estimatedRisk ?? 'low';
   ...
   */
  async validateModeTransition(
    context: ModeTransitionContext
  ): Promise<ModeTransitionResult> {
    const startTime = Date.now();
    
    // Get constraint sets for both modes
    const fromSets = await this.store.getConstraintSets(
      context.userId,
      context.fromMode,
      context.persona
    );
    const toSets = await this.store.getConstraintSets(
      context.userId,
      context.toMode,
      context.persona
    );
    ...
    const blockedBy: ModeTransitionResult['blockedBy'] = [];
    ...
    const appliedConstraints = success ? toSets.flatMap(set => 
      set.constraints.filter(c => c.active).map(c => c.id)
    ) : undefined;
   ...
   */
  private collectActiveConstraints(sets: ConstraintSet[], context: ActionContext): Constraint[] {
  ...
  private evaluateSingleConstraint(
    ...
    context: ActionContext
  ): { triggered: boolean; reason: string; riskFactor?: string } {
   ...
   */
  private isRiskAllowed(actionRisk: RiskLevel, maxRisk: RiskLevel): boolean {
   ...
   */
  private isInTimeWindow(window: { start: string; end: string }, timestamp?: Date): boolean {
   ...
   */
  private assessEffectiveRisk(baseRisk: RiskLevel, factorCount: number): RiskLevel {
    const riskLevels: RiskLevel[] = ['low', 'medium', 'high', 'critical'];
 ...
 */
export async function createConstraintEvaluator(store: IConstraintStore): Promise<DefaultConstraintEvaluator> {
