 ...
 */
import { OrbPersona, OrbDevice, OrbMode, } from '@orb-system/core-orb';
/**
...
const DEFAULT_RULES = [
    ...
    // Feature-based rules
    {
        id: 'feature-swl',
        weight: 10,
        matcher: (ctx) => ctx.feature?.toLowerCase().includes('snow white laundry') ||
    ...
    },
    {
        ...
        weight: 10,
        matcher: (ctx) => ctx.feature?.toLowerCase().includes('real estate') ||
    ...
    },
    {
        id: 'feature-personal',
        weight: 9,
        matcher: (ctx) => ctx.feature?.toLowerCase().includes('personal') ||
    ...
    // Event type rules
    {
        ...
        weight: 6,
        matcher: (ctx) => ctx.eventType === 'calendar_event' &&
    ...
    },
    {
        ...
        weight: 5,
        matcher: (ctx) => ctx.eventType === 'message' &&
 ...
 */
export function classifyPersona(context, customRules = []) {
    ...
    const secondHighest = Math.max(...Object.entries(scores)
...
class PersonaOverrideStore {
    ...
    setOverride(override) {
        ...
        const filtered = userOverrides.filter(o => {
            if (override.context) {
                return !(o.context?.deviceId === override.context.deviceId &&
    ...
    getOverride(userId, context) {
        ...
        for (const override of userOverrides) {
            ...
            // Check context match
            if (override.context) {
                const matches = (!override.context.deviceId || override.context.deviceId === context.deviceId) &&
    ...
    clearOverride(userId, context) {
        ...
        const filtered = userOverrides.filter(o => {
            if (!o.context)
                return true;
            return !(o.context.deviceId === context.deviceId &&
 ...
 */
export function setPersonaOverride(userId, persona, options) {
...
//# sourceMappingURL=personaClassification.js.map