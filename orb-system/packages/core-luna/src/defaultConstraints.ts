 ...
 */
export function getSystemDefaultConstraints(): Constraint[] {
  return [
    {
 ...
 */
export function getModeDefaultConstraintSets(): ConstraintSet[] {
 ...
 */
export function getAllDefaultConstraintSets(): ConstraintSet[] {
  const now = new Date().toISOString();
  
  // Wrap system defaults in a constraint set
  const systemSet: ConstraintSet = {
    id: 'default:system',
...
export async function initializeWithDefaults(
  store: { saveConstraintSet: (set: ConstraintSet) => Promise<void> }
): Promise<void> {
