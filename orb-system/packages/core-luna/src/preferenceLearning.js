...
export class PreferenceLearning {
     ...
     */
    async generateLearningActions(pattern) {
        const actions = [];
        switch (pattern.type) {
     ...
     */
    learnFromFrequentAction(pattern) {
     ...
     */
    learnFromModePreference(pattern) {
        if (pattern.confidence < 0.85)
            return [];
        const usageRate = pattern.data.usageRate ?? 0;
        return [{
                id: `action-${pattern.id}`,
     ...
     */
    learnFromRiskThreshold(pattern) {
     ...
     */
    learnFromTimeBasedRoutine(pattern) {
     ...
     */
    learnFromEfficiencyGain(pattern) {
        if (pattern.confidence < 0.75)
            return [];
        return [{
                id: `action-${pattern.id}`,
                ...
                suggestedValue: pattern.data.modes?.[0],
                reason: `New workflow is ${((1 - pattern.data.avgDuration) * 100).toFixed(0)}% faster`,
                status: 'pending',
            }];
    }
    /**
     ...
     */
    async applyLearningAction(action, profile) {
        switch (action.type) {
            case 'update_preference':
                ...
                profile.preferences = Object.entries(prefsRecord).map(([key, value]) => typeof value === 'boolean' ? key : `${key}: ${value}`);
                break;
            ...
            case 'adjust_risk_threshold':
                ...
                profile.preferences = Object.entries(riskPrefsRecord).map(([key, value]) => typeof value === 'boolean' ? key : `${key}: ${value}`);
                break;
