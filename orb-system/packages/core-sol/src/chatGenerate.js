 ...
 */
export async function generateChatResponse(ctx, request) {
    ...
    const systemPrompt = buildSystemPrompt(request.persona, request.mode, request.emotion, request.emotion_intensity, request.emotion_valence, request.emotion_energy, request.intent, request.ai_context);
    ...
    const bodyLanguage = await generateBodyLanguage(reply, request.emotion, request.emotion_intensity, request.emotion_valence, request.intent, openaiApiKey);
    return {
        reply,
        body_language: bodyLanguage,
    };
}
function buildSystemPrompt(persona, mode, emotion, intensity, valence, energy, intent, aiContext) {
    ...
    if (aiContext?.memory_stitch) {
        ...
        if (stitch.relevant_reflections && stitch.relevant_reflections.length > 0) {
            memoryContext = `\nRelevant Past Context: ${stitch.relevant_reflections.map((r) => r.context.substring(0, 100)).join('; ')}`;
        }
...
function getEmotionContext(emotion, intensity, valence, energy) {
...
async function generateBodyLanguage(reply, emotion, intensity, valence, intent, openaiApiKey) {
    if (!openaiApiKey) {
        return {
...
//# sourceMappingURL=chatGenerate.js.map